  ...
  try {
    ...
    const [
      ...
      supabase
        ...
        .select('occupation,monthly_income_range,monthly_income_exact,financial_goal,knowledge_level,risk_tolerance,other_notes,family_status,age_range,monthly_savings_percent,emergency_fund_status,investment_horizon,investment_priority,preferred_assets')
        .eq('user_id', userId)
        .maybeSingle(),
      supabase
        .from('user_portfolio')
    ...
    try {
      ...
      const isTpmError = msg.includes('Request too large') || msg.includes('tokens per minute') || (primaryErr as any)?.statusCode === 413
        || msg.includes('requires more credits') || msg.includes('can only afford');
      ...
      console.warn('[chat] Token/credit limit hit — retrying with fast model');
      result = await streamText({
        model: getModel(true),
        system: SYSTEM_PROMPT,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
