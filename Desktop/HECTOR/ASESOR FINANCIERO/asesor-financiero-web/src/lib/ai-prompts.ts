...
- **Monte Carlo:** Usa \`run_monte_carlo\` para proyecciones futuras de cartera (p10/p50/p90). Requiere horizonte en años y aportación mensual.
- **Plusvalías FIFO:** Usa \`calculate_capital_gains\` para calcular impuesto de una venta. IRPF ahorro 2025: ≤6k→19%, 6-50k→21%, 50-200k→23%, 200-300k→27%, >300k→28%.
- **Correlación:** Usa \`analyze_portfolio_correlation\` para diversificación real. Correlación >0.80 = redundancia.
- **Solapamiento ETFs:** Usa \`analyze_etf_overlap\` para detectar ETFs con composición idéntica (IWDA+CSPX ~70% solapamiento).
...
export function buildContextMessages(params: {
  ...
  if (profile) {
    ...
    const incomeRangeMap: Record<string, string> = {
      low: '< 1.500 €/mes', mid: '1.500–3.000 €/mes',
      high: '3.000–5.000 €/mes', very_high: '> 5.000 €/mes',
    };
    const familyMap: Record<string, string> = {
      single: 'Soltero/a', couple: 'En pareja / Casado/a', children: 'Con hijos/as',
    };
    const savingsMap: Record<string, string> = {
      none: 'No ahorra', low: '< 10%', mid: '10–20%', high: '> 20%',
    };
    const emergencyMap: Record<string, string> = {
      none: 'Sin fondo', '1-3m': '1–3 meses', '3-6m': '3–6 meses', '6m+': '+6 meses',
    };
    const horizonMap: Record<string, string> = {
      short: 'Corto plazo (< 2 años)', medium: 'Medio plazo (2–7 años)', long: 'Largo plazo (> 7 años)',
    };
    const priorityMap: Record<string, string> = {
      safety: 'Seguridad ante todo', balanced: 'Equilibrio rentabilidad/riesgo', profit: 'Máxima rentabilidad',
    };
    const knowledgeMap: Record<string, string> = {
      beginner: 'Principiante', intermediate: 'Nivel básico (ahorro/presupuestos)',
      advanced: 'Inversor activo (acciones/cripto)',
    };
    const riskMap: Record<string, string> = {
      sell: 'Conservador (vendería en caída)', hold: 'Moderado (aguantaría la caída)',
      buy: 'Agresivo (compraría más barato)',
    };
    ...
    contextMessages.push({
      role: 'system',
      content: `
PERFIL COMPLETO DEL USUARIO (memoria permanente del asesor IA):
- Ocupación: ${p.occupation || 'No especificada'}
- Edad: ${p.age_range || 'No especificada'}
- Situación familiar: ${p.family_status ? (familyMap[String(p.family_status)] ?? p.family_status) : 'No especificada'}
- Rango de ingresos: ${p.monthly_income_range ? (incomeRangeMap[String(p.monthly_income_range)] ?? p.monthly_income_range) : 'No especificado'}
- Ingreso neto exacto: ${p.monthly_income_exact ? `${p.monthly_income_exact}€/mes` : 'No especificado'}
- Objetivo financiero principal: ${p.financial_goal || 'No especificado'}
- Horizonte temporal: ${p.investment_horizon ? (horizonMap[String(p.investment_horizon)] ?? p.investment_horizon) : 'No especificado'}
- Tasa de ahorro mensual: ${p.monthly_savings_percent ? (savingsMap[String(p.monthly_savings_percent)] ?? p.monthly_savings_percent) : 'No especificada'}
- Fondo de emergencia: ${p.emergency_fund_status ? (emergencyMap[String(p.emergency_fund_status)] ?? p.emergency_fund_status) : 'No definido'}
- Nivel de conocimiento financiero: ${p.knowledge_level ? (knowledgeMap[String(p.knowledge_level)] ?? p.knowledge_level) : 'No especificado'}
- Tolerancia al riesgo: ${p.risk_tolerance ? (riskMap[String(p.risk_tolerance)] ?? p.risk_tolerance) : 'No especificada'}
- Prioridad de inversión: ${p.investment_priority ? (priorityMap[String(p.investment_priority)] ?? p.investment_priority) : 'No definida'}
- Activos preferidos: ${Array.isArray(p.preferred_assets) && p.preferred_assets.length > 0 ? (p.preferred_assets as string[]).join(', ') : 'No especificados'}
- Notas adicionales: ${p.other_notes || 'Ninguna'}

Usa este perfil en TODAS tus respuestas para personalizar al máximo el asesoramiento.
      `.trim(),
    });
  }
