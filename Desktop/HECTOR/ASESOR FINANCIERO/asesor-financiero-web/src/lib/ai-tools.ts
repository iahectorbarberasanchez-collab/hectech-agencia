...
import {
  ...
  getAnalystEstimates, getInsiderTrades, getSECFilings, searchFinancialNews,
} from './research-agent';
...
export function buildAITools(params: {
  ...
  return {
    ...
    // ── DEXTER RESEARCH TOOLS ──────────────────────────────────────────────

    get_income_statement: {
      description: 'Obtiene la cuenta de resultados (income statement) de una empresa: ingresos, beneficio neto, EBITDA, EPS. Úsalo cuando el usuario pregunte por los fundamentales de ingresos, márgenes o rentabilidad de una empresa.',
      inputSchema: z.object({
        ticker: z.string().describe('Ticker de la empresa (ej: AAPL, MSFT, NVDA)'),
        period: z.enum(['annual', 'quarterly']).optional().describe('annual o quarterly (default: annual)'),
        limit: z.number().optional().describe('Número de periodos a devolver (default: 4)'),
      }),
      execute: async ({ ticker, period = 'annual', limit = 4 }: { ticker: string; period?: 'annual' | 'quarterly'; limit?: number }) => {
        try {
          const data = await getIncomeStatements(ticker.toUpperCase(), period, limit);
          if (!data.length) return { success: false, error: `No se encontraron datos de income statement para ${ticker}.` };
          return { success: true, ticker: ticker.toUpperCase(), period, data };
        } catch (err: unknown) {
          return { success: false, error: err instanceof Error ? err.message : String(err) };
        }
      },
    ...
    get_balance_sheet: {
      description: 'Obtiene el balance de situación (balance sheet) de una empresa: activos totales, deuda, patrimonio neto, caja disponible. Úsalo para analizar la solidez financiera y nivel de apalancamiento.',
      inputSchema: z.object({
        ticker: z.string().describe('Ticker de la empresa'),
        period: z.enum(['annual', 'quarterly']).optional(),
        limit: z.number().optional(),
      }),
      execute: async ({ ticker, period = 'annual', limit = 4 }: { ticker: string; period?: 'annual' | 'quarterly'; limit?: number }) => {
        try {
          const data = await getBalanceSheets(ticker.toUpperCase(), period, limit);
          if (!data.length) return { success: false, error: `No se encontraron datos de balance para ${ticker}.` };
          return { success: true, ticker: ticker.toUpperCase(), period, data };
        } catch (err: unknown) {
          return { success: false, error: err instanceof Error ? err.message : String(err) };
        }
    ...
    get_cash_flow: {
      description: 'Obtiene el estado de flujos de caja de una empresa: flujo operativo, flujo libre (FCF), capex y dividendos. Fundamental para evaluar la generación real de caja vs. beneficio contable.',
      inputSchema: z.object({
        ticker: z.string().describe('Ticker de la empresa'),
        period: z.enum(['annual', 'quarterly']).optional(),
        limit: z.number().optional(),
      ...
      execute: async ({ ticker, period = 'annual', limit = 4 }: { ticker: string; period?: 'annual' | 'quarterly'; limit?: number }) => {
        try {
          const data = await getCashFlowStatements(ticker.toUpperCase(), period, limit);
          if (!data.length) return { success: false, error: `No se encontraron datos de flujo de caja para ${ticker}.` };
          return { success: true, ticker: ticker.toUpperCase(), period, data };
        } catch (err: unknown) {
          return { success: false, error: err instanceof Error ? err.message : String(err) };
        }
    ...
    get_analyst_estimates: {
      description: 'Obtiene las estimaciones de analistas (consenso) para ingresos y BPA (EPS) de una empresa. Útil para comparar expectativas del mercado con resultados reales.',
      inputSchema: z.object({
        ticker: z.string().describe('Ticker de la empresa'),
        period: z.enum(['annual', 'quarterly']).optional(),
        limit: z.number().optional(),
      ...
      execute: async ({ ticker, period = 'annual', limit = 4 }: { ticker: string; period?: 'annual' | 'quarterly'; limit?: number }) => {
        try {
          const data = await getAnalystEstimates(ticker.toUpperCase(), period, limit);
          if (!data.length) return { success: false, error: `No se encontraron estimaciones de analistas para ${ticker}.` };
          return { success: true, ticker: ticker.toUpperCase(), data };
        } catch (err: unknown) {
          return { success: false, error: err instanceof Error ? err.message : String(err) };
        }
      },
    ...
    get_insider_trades: {
      description: 'Obtiene las operaciones de insiders (compras y ventas de directivos y grandes accionistas) de una empresa. Una señal fundamental del sentiment interno de la compañía.',
      inputSchema: z.object({
        ticker: z.string().describe('Ticker de la empresa'),
        limit: z.number().optional().describe('Número de operaciones a devolver (default: 10)'),
      }),
      execute: async ({ ticker, limit = 10 }: { ticker: string; limit?: number }) => {
        try {
          const data = await getInsiderTrades(ticker.toUpperCase(), limit);
          if (!data.length) return { success: false, error: `No se encontraron operaciones de insiders para ${ticker}.` };
          return { success: true, ticker: ticker.toUpperCase(), data };
        } catch (err: unknown) {
          return { success: false, error: err instanceof Error ? err.message : String(err) };
        }
    ...
    get_sec_filings: {
      description: 'Obtiene los filings SEC más recientes de una empresa (10-K anual, 10-Q trimestral, 8-K eventos relevantes). Los 10-K y 10-Q contienen los estados financieros auditados completos.',
      inputSchema: z.object({
        ticker: z.string().describe('Ticker de la empresa'),
        filing_type: z.enum(['10-K', '10-Q', '8-K']).optional().describe('Tipo de filing SEC (default: 10-K)'),
        limit: z.number().optional().describe('Número de filings a devolver (default: 5)'),
      }),
      execute: async ({ ticker, filing_type = '10-K', limit = 5 }: { ticker: string; filing_type?: '10-K' | '10-Q' | '8-K'; limit?: number }) => {
        try {
          const data = await getSECFilings(ticker.toUpperCase(), filing_type, limit);
          if (!data.length) return { success: false, error: `No se encontraron filings ${filing_type} para ${ticker}.` };
          return { success: true, ticker: ticker.toUpperCase(), filingType: filing_type, data };
        } catch (err: unknown) {
          return { success: false, error: err instanceof Error ? err.message : String(err) };
        }
      },
    ...
    search_financial_news: {
      description: 'Realiza una búsqueda semántica de noticias y research financiero reciente mediante Exa. Úsalo cuando el usuario pregunte por noticias de una empresa, sector, o evento macroeconómico reciente.',
      inputSchema: z.object({
        query: z.string().describe('Consulta en lenguaje natural (ej: "NVIDIA earnings Q1 2025", "BCE tipos de interés junio 2025")'),
        num_results: z.number().optional().describe('Número de resultados (default: 5, max: 10)'),
      }),
      execute: async ({ query, num_results = 5 }: { query: string; num_results?: number }) => {
        try {
          const data = await searchFinancialNews(query, Math.min(num_results, 10));
          if (!data.length) return { success: false, error: 'No se encontraron resultados para esa búsqueda.' };
